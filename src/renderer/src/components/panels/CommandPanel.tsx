...
function classifyLine(line: string, type: string): string {
  ...
  if (l.includes('error') || l.includes('✗') || l.includes('failed') || type === 'stderr')
  ...
  if (l.includes('account') || l.includes('private key') || l.includes('balance'))
...
export default function CommandPanel({
  ...
  const handleRestart = useCallback(async () => {
    onStop(activeCommandId);
    await new Promise((r) => setTimeout(r, 800));
    onRun(activeCommandId);
  }, [activeCommandId, onStop, onRun]);
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      {/* Tab bar */}
      <div className="flex items-end flex-shrink-0 h-10 gap-px px-2 overflow-x-auto border-b bg-card border-border">
        {commands.map((cmd) => {
          ...
          return (
            <button
              ...
              {isRun && (
                <span
                  className="w-1.5 h-1.5 rounded-full animate-pulse-glow flex-shrink-0"
                  style={{ background: cmd.color }}
                />
              )}
              {isErr && <AlertCircle className="flex-shrink-0 w-3 h-3 text-rose-400" />}
              <span>{cmd.label}</span>
            </button>
          );
        })}

        {/* Hardhat Node quick-toggle */}
        <div className="flex items-end flex-shrink-0 pb-1 pr-1 ml-auto">
          <button
      ...
      {/* Command config bar */}
      <div className="flex items-center flex-shrink-0 gap-2 px-4 py-2 border-b bg-card border-border">
        <span
          className="flex-shrink-0 w-2 h-2 rounded-full ring-1 ring-border/50"
          style={{ background: activeCmd?.color }}
        />
        ...
        {editingId === activeCommandId ? (
          ...
          <div
            className={cn(
              ...
              !running &&
                'cursor-pointer hover:border-border/70 hover:bg-accent/30 transition-colors',
            ...
            {!running && (
              <PenLine className="w-3 h-3 opacity-0 text-muted-foreground/40 hover:opacity-100" />
            )}
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {running ? (
            <Button
              variant="destructive"
              size="sm"
              onClick={() => onStop(activeCommandId)}
              className="gap-1">
          ...
          <Button
            ...
            size="icon"
            className="w-7 h-7"
            onClick={() => onClear(activeCommandId)}>
      ...
      <div
        ...
        {filteredLogs.map((entry) => {
          ...
          return lines.map((line, i) => (
            <div
              key={`${entry.id}-${i}`}
              className={cn('whitespace-pre-wrap break-all', classifyLine(line, entry.type))}>
              {line}
            </div>
          ));
        })}
