  ...
  for (const [name, oldFn] of oldFnMap) {
    ...
    if (oldInputs !== newInputs) {
      diffs.push({
        ...
        oldSig: getFullSig(oldFn),
        newSig: getFullSig(newFn),
      });
    }
    ...
    if (oldOutputs !== newOutputs) {
      diffs.push({
        ...
        oldSig: `returns (${oldOutputs})`,
        newSig: `returns (${newOutputs})`,
      });
    }
    ...
    if (oldFn.stateMutability !== newFn.stateMutability) {
      const isCritical =
        (oldFn.stateMutability === 'view' || oldFn.stateMutability === 'pure') !==
        (newFn.stateMutability === 'view' || newFn.stateMutability === 'pure');
      diffs.push({
        type: 'mutability_changed',
        ...
        oldSig: oldFn.stateMutability,
        newSig: newFn.stateMutability,
      });
    }
    ...
    if (!newEvMap.has(name)) {
      ...
      if (getSig(oldEv) !== getSig(newEv)) {
        diffs.push({
          ...
          name: name ?? '',
          detail:
            'Event signature changed. topic0 hash is different; all event filters will stop working.',
          oldSig: getFullSig(oldEv),
          newSig: getFullSig(newEv),
        });
      }
    }
  }
...
export default function ABICompatibilityChecker({ abis, projectPath }: Props) {
  ...
  const severityIcon = (s: Severity) => {
    if (s === 'critical')
      return <XCircle className="w-3.5 h-3.5 text-rose-400 flex-shrink-0 mt-0.5" />;
    if (s === 'warning')
        ...
        <div className="flex flex-col flex-shrink-0 w-64 overflow-y-auto border-r border-border bg-card/50">
          <div className="p-3 space-y-2 border-b border-border">
            ...
            <select
              ...
              onChange={(e) => {
                setSelectedAbi(abis.find((a) => a.path === e.target.value) || null);
                setDiffs(null);
              }}
          ...
          <div className="flex-1 p-3 space-y-2 border-b border-border">
            ...
            <p className="text-[10px] text-muted-foreground/40">
              Paste the old ABI JSON array to detect breaking changes
            </p>
            <textarea
              value={oldAbiJson}
              onChange={(e) => {
                setOldAbiJson(e.target.value);
                setDiffs(null);
              }}
            ...
            <div className="p-3 space-y-2">
              ...
              {(['all', 'critical', 'warning', 'info'] as const).map((f) => (
                <button
                  ...
                  onClick={() => setFilter(f)}
                  className={cn(
                    'w-full text-left px-2 py-1.5 rounded text-xs flex items-center justify-between transition-all',
                    filter === f
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          {diffs !== null && diffs.length > 0 && (
            <div className="flex items-center gap-3 px-4 py-2 border-b border-border bg-card/30">
              ...
              {criticals > 0 && (
                <span className="ml-auto text-[10px] text-rose-400/70 bg-rose-500/10 px-2 py-0.5 rounded">
                  ⚠ Frontend will break
                </span>
              )}
            </div>
              ...
              {diffs === null ? (
                <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
                  ...
                  <p className="text-sm text-muted-foreground/40">
                    Select current ABI and paste old ABI to detect breaking changes
                  </p>
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-12">
                  ...
                  <p className="text-sm text-emerald-400">
                    {diffs.length === 0
                      ? 'ABIs are fully compatible — no breaking changes'
                ...
                filtered.map((diff, i) => (
                  <div
                    key={i}
                    className={cn('rounded-lg border p-3 space-y-2', severityStyle(diff.severity))}>
                    <div className="flex items-start gap-2">
                      {severityIcon(diff.severity)}
                      <div className="flex-1 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-mono text-xs font-semibold">{diff.name}</span>
                          <span
                            className={cn(
                        ...
                        {(diff.oldSig || diff.newSig) && (
                          <div className="space-y-0.5 mt-1">
                            {diff.oldSig && (
                              <div className="flex items-start gap-2">
                                <span className="text-[9px] text-rose-400/60 w-8 flex-shrink-0 mt-0.5">
                                  OLD
                                </span>
                                <code className="text-[10px] font-mono text-rose-300/60 line-through">
                                  {diff.oldSig}
                                </code>
                              </div>
                            )}
                            {diff.newSig && (
                              <div className="flex items-start gap-2">
                                <span className="text-[9px] text-emerald-400/60 w-8 flex-shrink-0 mt-0.5">
                                  NEW
                                </span>
                                <code className="text-[10px] font-mono text-emerald-300/60">
                                  {diff.newSig}
                                </code>
                              </div>
                            )}
