...
export default function ArtifactDiffPanel({ projectPath }: Props) {
  ...
  const computeDiff = (old: ArtifactMeta[], now: ArtifactMeta[]) => {
    ...
    const allNames = new Set([
      ...old.map((a) => a.contractName),
      ...now.map((a) => a.contractName),
    ]);
    for (const name of allNames) {
      ...
      const oldSigs = new Set(
        (o?.abi || [])
          .filter((i) => i.name)
          .map((i) => `${i.type}:${i.name}:${i.stateMutability || ''}`),
      );
  ...
  const allChanged = diffs.filter(
    (d) => d.abiAdded.length > 0 || d.abiRemoved.length > 0 || d.bytecodeSizeDelta !== 0,
  );
  ...
  const changed = diffs.filter(
    (d) =>
      d.bytecodeSizeOld > 0 &&
      d.bytecodeSizeNew > 0 &&
      (d.abiAdded.length || d.abiRemoved.length || d.bytecodeSizeDelta),
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 px-4 py-3 border-b border-border bg-card/50">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] text-muted-foreground/40">
            Compare compiled contract ABIs & bytecode sizes
          </span>
        </div>
        <div className="flex items-center gap-3">
          {/* Old snapshot */}
          <div className="flex-1 p-3 border rounded-lg border-border bg-muted/10">
            <div className="flex items-center justify-between mb-2">
              <div>
                <div className="text-xs font-semibold">Baseline</div>
                <div className="text-[10px] text-muted-foreground/50">
                  {oldLoaded
                    ? `${oldArtifacts.length} contracts loaded`
                    : 'Compile, then capture baseline'}
                </div>
              </div>
              <Button
                size="sm"
            ...
            {oldLoaded && (
              <div className="flex flex-wrap gap-1">
                {oldArtifacts.slice(0, 5).map((a) => (
                  <span
                    key={a.contractName}
                    className="text-[9px] bg-muted/30 px-1.5 py-0.5 rounded text-muted-foreground/60">
                    {a.contractName}
                  </span>
                ))}
                {oldArtifacts.length > 5 && (
                  <span className="text-[9px] text-muted-foreground/40">
                    +{oldArtifacts.length - 5} more
                  </span>
                )}
              </div>
            )}
          ...
          <ArrowRight className="flex-shrink-0 w-4 h-4 text-muted-foreground/30" />
          ...
          <div className="flex-1 p-3 border rounded-lg border-border bg-muted/10">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <div className="text-[10px] text-muted-foreground/50">
                  {newLoaded ? `${newArtifacts.length} contracts loaded` : 'Compile, then compare'}
                </div>
              </div>
              <Button
                size="sm"
                className="h-7 text-xs gap-1.5 bg-cyan-600 hover:bg-cyan-500"
                onClick={loadNew}
                disabled={!projectPath || loading || !oldLoaded}>
            ...
            {newLoaded && (
              <div className="flex flex-wrap gap-1">
                ...
                {newArtifacts.length > 5 && (
                  <span className="text-[9px] text-muted-foreground/40">
                    +{newArtifacts.length - 5} more
                  </span>
                )}
              </div>
            )}
      ...
      {/* Summary bar */}
      {diffs.length > 0 && (
        <div className="flex flex-shrink-0 gap-px border-b border-border">
          {[
      ...
      <ScrollArea className="flex-1 overflow-y-auto">
        {!oldLoaded ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-20 text-muted-foreground/30">
            ...
            <ol className="space-y-1 text-xs text-center opacity-60">
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2 py-20 text-muted-foreground/30">
                    ...
                    {!isNew && !isRemoved && (
                      <div className="flex items-center gap-3 text-[10px] font-mono">
                        <span className="text-muted-foreground/50">
                          {diff.bytecodeSizeOld.toLocaleString()}B
                        </span>
                        <ArrowRight className="w-3 h-3 text-muted-foreground/30" />
                        <span className="text-muted-foreground/70">
                          {diff.bytecodeSizeNew.toLocaleString()}B
                        </span>
                        <span className={cn('font-semibold', deltaColor)}>
                          {diff.bytecodeSizeDelta > 0 ? '+' : ''}
                          {diff.bytecodeSizeDelta}B
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="px-3 py-2 space-y-1.5">
                      {diff.abiAdded.map((fn) => (
                        <div key={fn} className="flex items-center gap-2 text-xs">
                          <Plus className="flex-shrink-0 w-3 h-3 text-emerald-400" />
                          <code className="font-mono text-emerald-300">{fn}</code>
                          <span className="text-[9px] text-emerald-500/60">added</span>
                        </div>
                      ))}
                      {diff.abiRemoved.map((fn) => (
                        <div key={fn} className="flex items-center gap-2 text-xs">
                          <Minus className="flex-shrink-0 w-3 h-3 text-rose-400" />
                          <code className="font-mono text-rose-300">{fn}</code>
