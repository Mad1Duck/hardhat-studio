...
function sendWcApproved(result) {
  if (result) {
    _wcLastResult = result;
    console.log("[WC] sendWcApproved →", result);
  }
    ...
    approval().then((session) => {
      try {
        ...
        if (accounts[0].includes(":")) {
          const p = accounts[0].split(":");
          addr = p[2];
          chainId = parseInt(p[1] || "1");
        } else {
          addr = accounts[0];
          chainId = 1;
        }
...
electron.ipcMain.handle("get-hardhat-accounts", async (_, rpcUrl) => {
  ...
  try {
    ...
    if (data.result && Array.isArray(data.result) && data.result.length > 0) {
      const liveAddresses = data.result;
