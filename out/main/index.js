...
function registerDiscordHandlers() {
  ...
  electron.ipcMain.handle("discord-check-role", async (_, { guildId, userId, roleIds }) => {
...
function registerFilesystemHandlers(getWin2) {
  ...
  electron.ipcMain.handle("read-env", async (_, folderPath) => {
    const envPath2 = path.join(folderPath, ".env");
    if (!fs.existsSync(envPath2)) return [];
    try {
      return fs.readFileSync(envPath2, "utf-8").split("\n").filter((l) => l.trim() && !l.startsWith("#")).map((l) => {
        const idx = l.indexOf("=");
